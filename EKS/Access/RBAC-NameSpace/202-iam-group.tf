...
resource "aws_iam_group_policy" "eks_admins_group_policy" {
  name  = "eks-admins-group-policy"
...
# a user in group
resource "aws_iam_user" "admin_user_group" {
  name          = "eksadmin-group"
  path          = "/"
...
resource "aws_iam_user_group_membership" "admin_user_group_membership" {
  user   = aws_iam_user.admin_user_group.name
  groups = [aws_iam_group.eks_admins_group.name]
}
